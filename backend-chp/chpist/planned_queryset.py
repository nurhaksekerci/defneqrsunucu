...
Rapor / planlı liste: ``start_gte``, ``start_lt`` (ISO 8601, ``start_at`` yarı açık aralık).
"""
...
def _parse_start_at_query(raw: str | None):
    """ISO 8601 (ofset dahil); geçersiz veya boş ise None."""
    if raw is None:
        return None
    s = str(raw).strip()
    if not s:
...
def apply_planned_start_at_range(
    qs: QuerySet[PlannedEvent], request
) -> QuerySet[PlannedEvent]:
    """Mobil rapor: ``start_gte`` ≤ ``start_at`` < ``start_lt``."""
    gte = _parse_start_at_query(_query_get(request, 'start_gte'))
    lt = _parse_start_at_query(_query_get(request, 'start_lt'))
    if gte is not None:
        qs = qs.filter(start_at__gte=gte)
    if lt is not None:
        qs = qs.filter(start_at__lt=lt)
    return qs


def planned_events_base_queryset(request) -> QuerySet[PlannedEvent]:
    """Durum filtresi yok; org kapsamı + kol/ilçe/kategori + isteğe bağlı ``start_at`` aralığı."""
    qs = PlannedEvent.objects.select_related('org_unit')
    scope = planned_events_scope_q_for_user(request.user)
    if scope is not None:
        qs = qs.filter(scope)
    qs = apply_feed_list_filters(qs, request.user, request)
    return apply_planned_start_at_range(qs, request)
