...
exports.getTables = async (req, res, next) => {
  try {
    ...
    if (!restaurantId) {
      return res.status(400).json({ success: false, message: 'Restoran ID gerekli' });
    }
...
exports.createBulkTables = async (req, res, next) => {
  try {
    ...
    if (isNaN(start) || isNaN(end) || start > end) {
      return res.status(400).json({ success: false, message: 'Geçersiz numara aralığı' });
    }

    if (end - start > 100) {
      return res.status(400).json({ success: false, message: 'Bir seferde en fazla 100 masa oluşturabilirsiniz' });
    }
    ...
    for (let i = start; i <= end; i++) {
      ...
      if (!existingNames.has(tableName)) {
        tablesToCreate.push({ name: tableName, restaurantId });
      }
    }

    if (tablesToCreate.length === 0) {
      return res.status(400).json({ success: false, message: 'Tüm masalar zaten mevcut' });
    }
    ...
    const createdTables = await prisma.table.findMany({
      where: {
        restaurantId,
        name: { in: tablesToCreate.map((t) => t.name) },
      },
    });
...
exports.createTable = async (req, res, next) => {
  try {
    ...
    if (!name || !restaurantId) {
      return res.status(400).json({ success: false, message: 'Masa adı ve restoran ID gerekli' });
    }
    ...
    if (error.code === 'P2002') {
      return res.status(400).json({ success: false, message: 'Bu masa adı zaten kullanılıyor' });
    }
    next(error);
  }
};
